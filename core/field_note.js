 ...
 * @constructor
 */
Blockly.FieldNote = function(noteNumber, opt_validator) {
    Blockly.FieldNote.superClass_.constructor.call(this, noteNumber,opt_validator);   
    };
...
Blockly.FieldNote.prototype.showEditor_ = function() {
    ...
    Blockly.FieldTextInput.htmlInput_.setAttribute('disabled', 'readonly');
    ...
    for (var j = 28; j < 64; j++) {
        ...
        if (black.indexOf(mod) == -1) {
            var frequenz = Math.round(440000.0 * Math.pow(2.0, (j - 49.0) / 12.0)) / 1000.0;
            var color = frequenz != this.getValue() ? '#ffffff' : Blockly.CAT_ACTION_RGB;
            ...
            Blockly.bindEvent_(note, 'mouseout', this, this.onMouseOut);
            i++;
        }
    }
    ...
    for (var j = 28; j < 64; j++) {
        ...
        if (black.indexOf(mod) >= 0) {
            var frequenz = Math.round(440000.0 * Math.pow(2.0, (j - 49.0) / 12.0)) / 1000.0;
            var color = frequenz != this.getValue() ? '#000000' : Blockly.CAT_ACTION_RGB;
               var note = Blockly.createSvgElement('rect', {
    ...
    // svg.style.marginLeft = (15 - Blockly.FieldNote.RADIUS) + 'px';
...
Blockly.FieldNote.prototype.onMouseHover = function(e) {
    ...
    var noteNumber = e.target.id.replace(/white|black/, '');
    this.setValue(noteNumber);
    this.resizeEditor_();
};

Blockly.FieldNote.prototype.onMouseOut = function(e) {
    if (e.fromElement) {
        if (e.fromElement.id.indexOf('white') != -1) {
            e.fromElement.setAttribute('fill', '#ffffff');
        } else {
            e.fromElement.setAttribute('fill', '#000000');
        }
    }
}
