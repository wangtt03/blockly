 ...
 * 
 * @param {!Object}
 *            workspaceOptions Dictionary of options for the workspace.
 * @constructor
 */
...
Blockly.Flyout.prototype.createDom = function() {
    /*
     * <g> <path class="blocklyFlyoutBackground"/> <g class="blocklyFlyout"></g>
     * </g>
     */
 ...
 *            targetWorkspace The workspace in which to create new blocks.
 */
 ...
 * flyout. The following properties are computed: .viewHeight: Height of the
 * visible rectangle, .viewWidth: Width of the visible rectangle,
 * .contentHeight: Height of the contents, .contentWidth: Width of the contents,
 * .viewTop: Offset of top edge of visible rectangle from parent, .contentTop:
 * Offset of the top-most content from the y=0 coordinate, .absoluteTop:
 * Top-edge of view. .viewLeft: Offset of the left edge of visible rectangle
 * from parent, .contentLeft: Offset of the left-most content from the x=0
 * coordinate, .absoluteLeft: Left-edge of view.
 * 
 ...
 * 
 * @param {!Object}
 *            xyRatio Contains a y property which is a float between 0 and 1
...
Blockly.Flyout.prototype.position = function() {
    ...
    this.setBackgroundPath_(edgeWidth, this.horizontalLayout_ ? this.height_ : metrics.viewHeight);
 ...
 * 
 * @param {number}
 *            width The width of the flyout, not including the rounded corners.
 * @param {number}
 *            height The height of the flyout, not including rounded corners.
 * @private
 */
...
Blockly.Flyout.prototype.wheel_ = function(e) {
    ...
    if (delta) {
        ...
        var metrics = this.getMetrics_();
        var pos = this.horizontalLayout_ ? metrics.viewLeft + delta : metrics.viewTop + delta;
        var limit = this.horizontalLayout_ ? metrics.contentWidth - metrics.viewWidth : metrics.contentHeight - metrics.viewHeight;
 ...
 * 
 * @param {!Array|string}
 *            xmlList List of blocks to show. Variables and procedures have a
 *            custom set of blocks.
 */
Blockly.Flyout.prototype.show = function(xmlList, opt_colour) {
    ...
    if (xmlList == Blockly.Variables.NAME_TYPE) {
        ...
        xmlList = Blockly.Variables.flyoutCategory(this.workspace_.targetWorkspace);
    } else if (xmlList == Blockly.Procedures.NAME_TYPE) {
        // Special category for procedures.
        xmlList = Blockly.Procedures.flyoutCategory(this.workspace_.targetWorkspace);
    }
 ...
 * 
 * @param {!Array.
 *            <!Blockly.BlockSvg>} blocks The blocks to lay out.
 * @param {!Array.
 ...
 * 
 * @param {!Element}
 *            root The root node of the SVG group the block is in.
 * @param {!Blockly.Block}
 *            block The block to add listeners for.
 * @param {!Element}
 *            rect The invisible rectangle under the block that acts as a button
 *            for that block.
 * @private
 */
Blockly.Flyout.prototype.addBlockListeners_ = function(root, block, rect) {
    if (this.autoClose) {
        ...
        this.listeners_.push(Blockly.bindEvent_(rect, 'mousedown', null, this.blockMouseDown_(block)));
    }
 ...
 * 
 * @param {!Blockly.Block}
 *            block The flyout block to copy.
...
Blockly.Flyout.prototype.blockMouseDown_ = function(block) {
    ...
    return function(e) {
        ...
        if (Blockly.isRightButton(e)) {
            ...
            Blockly.Flyout.onMouseUpWrapper_ = Blockly.bindEvent_(document, 'mouseup', this, flyout.onMouseUp_);
            Blockly.Flyout.onMouseMoveBlockWrapper_ = Blockly.bindEvent_(document, 'mousemove', this, flyout.onMouseMoveBlock_);
        }
...
Blockly.Flyout.prototype.onMouseDown_ = function(e) {
    ...
    Blockly.Flyout.onMouseMoveWrapper_ = Blockly.bindEvent_(document, 'mousemove', this, this.onMouseMove_);
    Blockly.Flyout.onMouseUpWrapper_ = Blockly.bindEvent_(document, 'mouseup', this, Blockly.Flyout.terminateDrag_);
    // This event has been handled.  No need to bubble up to the document.
    e.preventDefault();
    e.stopPropagation();
};
...
/**
 * Mouse button is down on a block in a non-closing flyout. Create the block if
 * the mouse moves beyond a small radius. This allows one to play with fields
 * without instantiating blocks that instantly self-destruct.
 * 
 * @param {!Event}
 *            e Mouse move event.
 * @private
 ...
 * 
 * @param {!Blockly.Block}
 *            originBlock The flyout block to copy.
 * @return {!Function} Function to call when block is clicked.
 * @private
 */
...
Blockly.Flyout.prototype.getClientRect = function() {
    ...
    if (this.toolboxPosition_ == Blockly.TOOLBOX_AT_TOP) {
        return new goog.math.Rect(-BIG_NUM, y - BIG_NUM, BIG_NUM * 2, BIG_NUM + height);
    } else if (this.toolboxPosition_ == Blockly.TOOLBOX_AT_BOTTOM) {
        return new goog.math.Rect(-BIG_NUM, y, BIG_NUM * 2, BIG_NUM + height);
...
/**
 * Compute height of flyout. Position button under each block. For RTL: Lay out
 * the blocks right-aligned.
 * 
 * @param {!Array
 *            <!Blockly.Block>} blocks The blocks to reflow.
 */
    ...
    if (this.height_ != flyoutHeight) {
        for (var i = 0, block; block = blocks[i]; i++) {
            ...
            if (block.flyoutRect_) {
                ...
                block.flyoutRect_.setAttribute('x', this.RTL ? blockXY.x - blockHW.width + tab : blockXY.x - tab);
...
/**
 * Compute width of flyout. Position button under each block. For RTL: Lay out
 * the blocks right-aligned.
 * 
 * @param {!Array
 *            <!Blockly.Block>} blocks The blocks to reflow.
...
Blockly.Flyout.prototype.offsetHorizontalRtlBlocks = function(blocks) {
    if (this.horizontalLayout_ && this.RTL) {
        ...
        var offset = Math.max(-optionBox.x + this.MARGIN, this.width_ / this.workspace_.scale);

        for (var i = 0, block; block = blocks[i]; i++) {
            block.moveBy(offset, 0);
            if (block.flyoutRect_) {
                block.flyoutRect_.setAttribute('x', offset + Number(block.flyoutRect_.getAttribute('x')));
            }
        }
    }
};
