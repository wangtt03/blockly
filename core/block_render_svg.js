 ...
 * @const
 */
Blockly.BlockSvg.NOTCH_PATH_LEFT = 'l 6,4 3,0 6,-4'; // h 2.5l 5, 5 5, -5 h 2.5
/**
 ...
 * @const
 */
Blockly.BlockSvg.NOTCH_PATH_RIGHT = 'l -6,4 -3,0 -6,-4'; //h -3.5l -4, 4 -4, -4 h -3.5
/**
    ...
    if (this.isCollapsed()) {
      ...
      for (var x = 0, input; input = row[x]; x++) {
        ...
        if (input.type == Blockly.INPUT_VALUE) {
          ...
          this.drawInputType_(input, y, cursorX - Blockly.BlockSvg.SEP_SPACE_X + Blockly.BlockSvg.TAB_WIDTH - 2 - input.renderWidth, cursorY
...
Blockly.BlockSvg.prototype.drawInputType_ = function(input, y, fieldX, fieldY) {
    if (input.connection.check_ && !input.connection.targetConnection && input.connection.check_.length ==  1) {
        var type = input.connection.check_[0];
        var color = Blockly.DATA_TYPE[type];
        var svgPathInput = Blockly.createSvgElement('path', {
            //'id' : this.id + '_' + this.svgPathInput_.length,
            'd' : 'M ' + (fieldX) + ',' + (fieldY + 2.6) + 'l-6.188 -0.547c0 0 -1.781 4.781 -1.938 7.281s0.578 4.594 0.984 '
        ...
        this.svgGroup_.insertBefore(this.svgPathInput_[this.svgPathInput_.length - 1], this.svgPathSelected_);
    }
};
