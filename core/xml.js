...
Blockly.Xml.domToBlock = function(workspace, xmlBlockList, opt_reuseBlock) {
    ...
    var topBlock = Blockly.Xml.domToBlockHeadless_(workspace, xmlBlockList, opt_reuseBlock);
 ...
 * @private
 */
Blockly.Xml.domToBlockHeadless_ = function(workspace, xmlBlockList, opt_reuseBlock) {
    ...
    for (var i = 0, xmlChild; xmlChild = xmlBlock.childNodes[i]; i++) {
        ...
        if (xmlChild.nodeName != 'repetitions') {
            Blockly.Xml.childToBlock(workspace, block, xmlChild, opt_reuseBlock, blockChild);
        } else {
            for (var u = 0, xmlRepetChild; xmlRepetChild = xmlChild.childNodes[u]; u++) {
                ...
                Blockly.Xml.childToBlock(workspace, block, xmlRepetChild, opt_reuseBlock);
            }
        }
    }
    ...
    if (xmlBlockList.constructor === Array) {
        ...
        if (nextBlockList.length != 0) {
            ...
            blockChild = Blockly.Xml.domToBlock(workspace, nextBlockList, opt_reuseBlock);
 ...
 */
Blockly.Xml.childToBlock = function(workspace, block, xmlChild, opt_reuseBlock, blockChild) {
...
// Use the shadow block if there is no child block.
    ...
    case 'statement':
        ...
        if (childBlockNode) {
            blockChild = Blockly.Xml.domToBlockHeadless_(workspace, childBlockNode, opt_reuseBlock);
